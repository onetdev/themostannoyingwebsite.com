...
The output MUST be saved as a **markdown artifact** in the `./artifacts/` directory.

## Guidelines

- **Format**: Use markdown in `artifacts/branch-summary.md`
...
4.  **Save Output**: Write the final summary to `./artifacts/branch-summary.md`.
5.  **Optional: Detailed Diff**: If requested, run `git diff <target>..HEAD` for a deeper look at the code changes.

## Best Practices

- **Artifacts over CLI**: Never output large summaries directly to the CLI. Always use `./artifacts/branch-summary.md`.
