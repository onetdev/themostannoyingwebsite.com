...
- **Skewed Progress:** The verification bar uses a logarithmic curve: `Math.log10(1 + 9 * (completed / 100)) * 100`. Because the system requires 20 completions (`requiredCompletedChallanges`) but the UI calculates progress relative to 100, the bar will only reach ~44% before the verification is considered "complete" and resolved.
