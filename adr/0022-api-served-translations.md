...
## Status
Accepted. Partially superseded by ADR 25 (bundled variant fallbacks removed; variant pools are now API-only and SSR-hydrated).
...
3.  **Variant pools come from the API.** Services use `getVariantPool()` (`client.variants.getByType`) and fall back to the bundled English variant arrays when the API is unavailable. *Superseded by ADR 25: bundled fallbacks were removed and pools are prefetched server-side into the React Query cache.*
